...
#+BEGIN_SRC clojure :tangle no :results value pp replace
...
(defn validate-cmd [m]
  (if (not (s/valid? ::transformation/command m))
    (do
      ...
      false) 
    true))
:akvo.lumen.transformation/command
(def res (partition 2 
                   ["core/sort-column"
...
(filter (comp false? second) res)
#+END_SRC

#+RESULTS:
: In: [:transformation] val: {:args {:source {:aggregationColumn [:v "c1"], :aggregationDirection "DESC", :datasetId "5b27ca89-221e-4e58-999f-da349713798b", :mergeColumn "c1", :mergeColumns ["c1"]}, :target {:mergeColumn "c1"}}, :akvo.lumen.transformation.engine/op "core/merge-datasets"} fails spec: :akvo.lumen.transformation/transformation at: [:transformation :transformation] predicate: valid?
: (("core/merge-datasets" false))
